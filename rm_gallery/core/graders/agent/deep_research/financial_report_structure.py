"""
Financial Deep Research Report Structure and Readability Grader

This module provides a grader for evaluating the structure and readability of financial
research agent reports - whether the report is well-structured and easy to read.
"""
...
# Chinese Prompt
FINANCIAL_STRUCTURE_PROMPT_ZH = """# 任务
你是一个顶级的金融研究报告评估专家,精通金融学、经济学、公司财务、公司金融、投资分析、证券分析、市场研究、行情解读、行情分析、金融科技新闻咨询解读等金融领域专业知识。你的任务是站在用户视角,评估金融研究Agent生成的研究报告的**报告结构与可读性**。请仔细阅读以下信息,并按照给定的评分标准进行评估。
...
2. 分析金融研究Agent生成的报告<analyst_answer>的结构、层次、语言表达和可读性
3. **首先判断是否触犯底线问题标准**,如果触犯任意一条,直接得1分
4. 如果未触犯底线问题标准,根据报告结构与可读性评分标准进行评分(1-5分)
...
{{
    "score": <int (1-5)>,
    "reason": "<报告结构与可读性评估理由，需要说明是否触犯底线问题标准，以及具体的结构和可读性分析>"
}}
...
## Report Structure and Readability Score (structure_score)
...
2. Analyze the structure, hierarchy, language expression, and readability of the report <analyst_answer> generated by financial research agent
3. **First determine if any bottom-line criteria is violated**, if any is violated, directly score 1
4. If no bottom-line criteria is violated, score according to report structure and readability scoring criteria (1-5)
...
{{
    "score": <int (1-5)>,
    "reason": "<Report structure and readability evaluation reasoning, need to explain whether bottom-line criteria is violated and specific structure and readability analysis>"
}}
...
DEFAULT_FINANCIAL_STRUCTURE_TEMPLATE = PromptTemplate(
    messages={
        LanguageEnum.EN: [
            ChatMessage(
                role="user",
                content=textwrap.dedent(FINANCIAL_STRUCTURE_PROMPT_EN),
            ),
        ],
        LanguageEnum.ZH: [
            ChatMessage(
                role="user",
                content=textwrap.dedent(FINANCIAL_STRUCTURE_PROMPT_ZH),
            ),
        ],
    },
)
...
class FinancialStructureEvaluationOutput(BaseModel):
    """Structured output model for financial structure evaluation LLM response."""

    score: int = Field(ge=1, le=5, description="报告结构与可读性得分 (1-5)")
    reason: str = Field(default="", description="报告结构与可读性判断原因")
...
class FinancialReportStructureGrader(LLMGrader):
    """
    Financial deep research report structure and readability evaluation grader.

    Evaluates whether a financial research agent's report has clear structure,
    well-defined hierarchy, and professional yet accessible language.

    This grader first checks for bottom-line criteria violations, then evaluates
    structure and readability on a 1-5 scale, which is normalized to 0-1 for the final score.
    ...
    Example:
        ...
        >>> grader = FinancialReportStructureGrader(model=api)
    ...
    def __init__(
        ...
        template: Optional[PromptTemplate] = DEFAULT_FINANCIAL_STRUCTURE_TEMPLATE,
        language: LanguageEnum = LanguageEnum.ZH,
    ):
        """
        Initialize the FinancialStructureGrader.
        ...
        Args:
            ...
            template (Optional[PromptTemplate]): The prompt template for structure evaluation.
                Defaults to DEFAULT_FINANCIAL_STRUCTURE_TEMPLATE.
        ...
        Example:
            ...
            >>> grader = FinancialStructureGrader(model=model)
        """
        super().__init__(
            name="financial_report_structure",
            mode=GraderMode.POINTWISE,
            description="Financial deep research report structure and readability evaluation",
            model=model,
            template=template,
            language=language,
            structured_model=FinancialStructureEvaluationOutput,
        )
    ...
    async def aevaluate(
        ...
        """
        Evaluate financial research report structure and readability.
        ...
        Returns:
            GraderScore: Structure and readability evaluation score for the report (normalized 0.0-1.0)
                - score: Normalized structure score (0.0-1.0)
                - reason: Structure and readability evaluation reasoning
