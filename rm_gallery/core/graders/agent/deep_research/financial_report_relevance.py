"""
Financial Deep Research Report Relevance Grader

This module provides a grader for evaluating the relevance of financial research
agent reports - whether the content is relevant to user's question and avoids irrelevant information.
"""
...
# Chinese Prompt
FINANCIAL_RELEVANCE_PROMPT_ZH = """# 任务
你是一个顶级的金融研究报告评估专家,精通金融学、经济学、公司财务、公司金融、投资分析、证券分析、市场研究、行情解读、行情分析、金融科技新闻咨询解读等金融领域专业知识。你的任务是站在用户视角,评估金融研究Agent生成的研究报告的**相关性**。请仔细阅读以下信息,并按照给定的评分标准进行评估。
...
2. 分析金融研究Agent生成的报告<analyst_answer>中的内容,识别哪些内容与用户问题相关,哪些无关
3. **首先判断是否触犯底线问题标准**,如果触犯任意一条,直接得1分
4. 如果未触犯底线问题标准,根据相关性评分标准进行评分(1-5分)
...
{{
    "score": <int (1-5)>,
    "reason": "<相关性评估理由，需要说明是否触犯底线问题标准，以及具体的相关性分析，指出相关和无关的内容>"
}}
...
2. Analyze the content in the report <analyst_answer> generated by financial research agent, identify which content is relevant and which is irrelevant to the user question
3. **First determine if any bottom-line criteria is violated**, if any is violated, directly score 1
4. If no bottom-line criteria is violated, score according to relevance scoring criteria (1-5)
...
{{
    "score": <int (1-5)>,
    "reason": "<Relevance evaluation reasoning, need to explain whether bottom-line criteria is violated and specific relevance analysis, pointing out relevant and irrelevant content>"
}}
...
DEFAULT_FINANCIAL_RELEVANCE_TEMPLATE = PromptTemplate(
    messages={
        LanguageEnum.EN: [
            ChatMessage(
                role="user",
                content=textwrap.dedent(FINANCIAL_RELEVANCE_PROMPT_EN),
            ),
        ],
        LanguageEnum.ZH: [
            ChatMessage(
                role="user",
                content=textwrap.dedent(FINANCIAL_RELEVANCE_PROMPT_ZH),
            ),
        ],
    },
)
...
class FinancialRelevanceEvaluationOutput(BaseModel):
    """Structured output model for financial relevance evaluation LLM response."""

    score: int = Field(ge=1, le=5, description="相关性得分 (1-5)")
    reason: str = Field(default="", description="相关性判断原因")
...
class FinancialReportRelevanceGrader(LLMGrader):
    """
    Financial deep research report relevance evaluation grader.

    Evaluates whether a financial research agent's report content is relevant
    to the user's question and avoids introducing irrelevant information.

    This grader first checks for bottom-line criteria violations, then evaluates
    relevance on a 1-5 scale, which is normalized to 0-1 for the final score.
    ...
    Example:
        ...
        >>> grader = FinancialReportRelevanceGrader(model=api)
    ...
    def __init__(
        ...
        template: Optional[PromptTemplate] = DEFAULT_FINANCIAL_RELEVANCE_TEMPLATE,
        language: LanguageEnum = LanguageEnum.ZH,
    ):
        """
        Initialize the FinancialRelevanceGrader.
        ...
        Args:
            ...
            template (Optional[PromptTemplate]): The prompt template for relevance evaluation.
                Defaults to DEFAULT_FINANCIAL_RELEVANCE_TEMPLATE.
        ...
        Example:
            ...
            >>> grader = FinancialRelevanceGrader(model=model)
        """
        super().__init__(
            name="financial_report_relevance",
            mode=GraderMode.POINTWISE,
            description="Financial deep research report relevance evaluation",
            model=model,
            template=template,
            language=language,
            structured_model=FinancialRelevanceEvaluationOutput,
        )
    ...
    async def aevaluate(
        ...
        """
        Evaluate financial research report relevance.
        ...
        Returns:
            GraderScore: Relevance evaluation score for the report (normalized 0.0-1.0)
                - score: Normalized relevance score (0.0-1.0)
                - reason: Relevance evaluation reasoning
