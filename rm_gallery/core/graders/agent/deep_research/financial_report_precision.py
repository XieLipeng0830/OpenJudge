"""
Financial Deep Research Report Precision Grader

This module provides a grader for evaluating the precision of financial research
agent reports - whether the report directly addresses the core request and matches
...
# Chinese Prompt
FINANCIAL_PRECISION_PROMPT_ZH = """# 任务
你是一个顶级的金融研究报告评估专家,精通金融学、经济学、公司财务、公司金融、投资分析、证券分析、市场研究、行情解读、行情分析、金融科技新闻咨询解读等金融领域专业知识。你的任务是站在用户视角,评估金融研究Agent生成的研究报告的**精准性**。请仔细阅读以下信息,并按照给定的评分标准进行评估。
...
4. 如果未触犯底线问题标准,根据精准性评分标准进行评分(1-5分)
...
{{
    "score": <int (1-5)>,
    "reason": "<精准性评估理由，需要说明是否触犯底线问题标准，以及具体的精准性分析>"
}}
...
4. If no bottom-line criteria is violated, score according to precision scoring criteria (1-5)
...
{{
    "score": <int (1-5)>,
    "reason": "<Precision evaluation reasoning, need to explain whether bottom-line criteria is violated and specific precision analysis>"
}}
...
DEFAULT_FINANCIAL_PRECISION_TEMPLATE = PromptTemplate(
    messages={
        LanguageEnum.EN: [
            ChatMessage(
                role="user",
                content=textwrap.dedent(FINANCIAL_PRECISION_PROMPT_EN),
            ),
        ],
        LanguageEnum.ZH: [
            ChatMessage(
                role="user",
                content=textwrap.dedent(FINANCIAL_PRECISION_PROMPT_ZH),
            ),
        ],
    },
)
...
class FinancialPrecisionEvaluationOutput(BaseModel):
    """Structured output model for financial precision evaluation LLM response."""

    score: int = Field(ge=1, le=5, description="精准性得分 (1-5)")
    reason: str = Field(default="", description="精准性判断原因")
...
class FinancialReportPrecisionGrader(LLMGrader):
    """
    Financial deep research report precision evaluation grader.

    Evaluates whether a financial research agent's report precisely addresses
    the user's question with accurate entity matching and focus.

    This grader first checks for bottom-line criteria violations, then evaluates
    precision on a 1-5 scale, which is normalized to 0-1 for the final score.
    ...
    def __init__(
        ...
        template: Optional[PromptTemplate] = DEFAULT_FINANCIAL_PRECISION_TEMPLATE,
        language: LanguageEnum = LanguageEnum.ZH,
    ):
        """
        Initialize the FinancialPrecisionGrader.
        ...
        Args:
            ...
            template (Optional[PromptTemplate]): The prompt template for precision evaluation.
                Defaults to DEFAULT_FINANCIAL_PRECISION_TEMPLATE.
        ...
        """
        super().__init__(
            name="financial_report_precision",
            mode=GraderMode.POINTWISE,
            description="Financial deep research report precision evaluation",
            model=model,
            template=template,
            language=language,
            structured_model=FinancialPrecisionEvaluationOutput,
        )
    ...
    async def aevaluate(
        ...
        """
        Evaluate financial research report precision.
        ...
        Returns:
            GraderScore: Precision evaluation score for the report (normalized 0.0-1.0)
                - score: Normalized precision score (0.0-1.0)
                - reason: Precision evaluation reasoning
