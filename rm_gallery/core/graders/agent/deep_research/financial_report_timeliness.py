"""
Financial Deep Research Report Timeliness Grader

This module provides a grader for evaluating the timeliness of financial research
agent reports - whether the report uses latest market information and data.
"""
...
# Chinese Prompt
FINANCIAL_TIMELINESS_PROMPT_ZH = """# 任务
你是一个顶级的金融研究报告评估专家,精通金融学、经济学、公司财务、公司金融、投资分析、证券分析、市场研究、行情解读、行情分析、金融科技新闻咨询解读等金融领域专业知识。你的任务是站在用户视角,评估金融研究Agent生成的研究报告的**时效性**。请仔细阅读以下信息,并按照给定的评分标准进行评估。
...
2. 分析金融研究Agent生成的报告<analyst_answer>中使用的数据和信息的时效性,结合报告生成日期<date>
3. **首先判断是否触犯底线问题标准**,如果触犯任意一条,直接得1分
4. 如果未触犯底线问题标准,根据时效性评分标准进行评分(1-5分)
...
{{
    "score": <int (1-5)>,
    "reason": "<时效性评估理由，需要说明是否触犯底线问题标准，以及具体的时效性分析，包括数据新旧程度>"
}}
...
2. Analyze the timeliness of data and information used in the report <analyst_answer> generated by financial research agent, in combination with report generation date <date>
3. **First determine if any bottom-line criteria is violated**, if any is violated, directly score 1
4. If no bottom-line criteria is violated, score according to timeliness scoring criteria (1-5)
...
{{
    "score": <int (1-5)>,
    "reason": "<Timeliness evaluation reasoning, need to explain whether bottom-line criteria is violated and specific timeliness analysis, including data recency>"
}}
...
DEFAULT_FINANCIAL_TIMELINESS_TEMPLATE = PromptTemplate(
    messages={
        LanguageEnum.EN: [
            ChatMessage(
                role="user",
                content=textwrap.dedent(FINANCIAL_TIMELINESS_PROMPT_EN),
            ),
        ],
        LanguageEnum.ZH: [
            ChatMessage(
                role="user",
                content=textwrap.dedent(FINANCIAL_TIMELINESS_PROMPT_ZH),
            ),
        ],
    },
)
...
class FinancialTimelinessEvaluationOutput(BaseModel):
    """Structured output model for financial timeliness evaluation LLM response."""

    score: int = Field(ge=1, le=5, description="时效性得分 (1-5)")
    reason: str = Field(default="", description="时效性判断原因")
...
class FinancialReportTimelinessGrader(LLMGrader):
    """
    Financial deep research report timeliness evaluation grader.

    Evaluates whether a financial research agent's report uses the latest
    market information, data, and analytical evidence.

    This grader first checks for bottom-line criteria violations, then evaluates
    timeliness on a 1-5 scale, which is normalized to 0-1 for the final score.
    ...
    Example:
        ...
        >>> grader = FinancialReportTimelinessGrader(model=api)
    ...
    def __init__(
        ...
        template: Optional[PromptTemplate] = DEFAULT_FINANCIAL_TIMELINESS_TEMPLATE,
        language: LanguageEnum = LanguageEnum.ZH,
    ):
        """
        Initialize the FinancialTimelinessGrader.
        ...
        Args:
            ...
            template (Optional[PromptTemplate]): The prompt template for timeliness evaluation.
                Defaults to DEFAULT_FINANCIAL_TIMELINESS_TEMPLATE.
        ...
        Example:
            ...
            >>> grader = FinancialTimelinessGrader(model=model)
        """
        super().__init__(
            name="financial_report_timeliness",
            mode=GraderMode.POINTWISE,
            description="Financial deep research report timeliness evaluation",
            model=model,
            template=template,
            language=language,
            structured_model=FinancialTimelinessEvaluationOutput,
        )
    ...
    async def aevaluate(
        ...
        """
        Evaluate financial research report timeliness.
        ...
        Returns:
            GraderScore: Timeliness evaluation score for the report (normalized 0.0-1.0)
                - score: Normalized timeliness score (0.0-1.0)
                - reason: Timeliness evaluation reasoning
        ...
        Example:
            ...
            ...     chat_date="2024-01-01"
            ... )
            >>> print(f"Timeliness Score: {result.score}")
        """
