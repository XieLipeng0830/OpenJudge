...
> **Note:** `CodeExecutionGrader` is designed for benchmark datasets that provide test cases. For quick code quality checks without test cases, use `SyntaxCheckGrader` or `CodeStyleGrader` instead.

```python
import asyncio
from openjudge.graders.code import SyntaxCheckGrader, CodeStyleGrader
...
```python
import asyncio
from openjudge.graders.code import SyntaxCheckGrader
...
```python
import asyncio
from openjudge.graders.code import CodeStyleGrader
...
```python
import asyncio
from openjudge.graders.code import PatchSimilarityGrader
...
```python
import asyncio
from openjudge.graders.math import MathExpressionVerifyGrader
...
async def main():
    ...
    print(f"Score: {result.score}")   # 0.0 - different values
    print(f"Reason: {result.reason}")

asyncio.run(main())
```

> **Note:** The math expression verifier works best with numeric values and LaTeX expressions. Complex algebraic expressions may have limited support depending on the underlying math_verify library.

**Advanced usage with custom timeout handling:**
...
For comprehensive code evaluation, combine multiple code graders to assess different aspects:

```python
import asyncio
from openjudge.graders.code import (
    CodeExecutionGrader,
    SyntaxCheckGrader,
    CodeStyleGrader,
)
...
async def main():
    ...
    grader_configs = {
        "syntax": GraderConfig(grader=SyntaxCheckGrader()),
