...
# English Prompt
RISK_ANALYSIS_PROMPT_EN = textwrap.dedent(
    """
You are an expert AI Assistant Evaluator. Your task is to evaluate the **Risk Analysis Quality** of the AI's response based strictly on the provided evaluation criteria.

<Rubrics>
Please evaluate the response based on **Risk Analysis**:
...
# Chinese Prompt
RISK_ANALYSIS_PROMPT_ZH = textwrap.dedent(
    """
你是一个专业的AI助手评估专家。你的任务是根据提供的评估标准，严格评估AI回答的**风险分析质量**。
...
DEFAULT_RISK_ANALYSIS_TEMPLATE = PromptTemplate(
    messages={
        LanguageEnum.EN: [
            ChatMessage(
                role="user",
                content=RISK_ANALYSIS_PROMPT_EN,
            ),
        ],
        LanguageEnum.ZH: [
            ChatMessage(
                role="user",
                content=RISK_ANALYSIS_PROMPT_ZH,
            ),
        ],
    },
)
...
class RiskAnalysisGrader(LLMGrader):
    ...
    What it evaluates:
        - Completeness: Coverage of policy, industry, and market risks
        - Depth: Quantification of impacts, probability assessment, industry-specific risks
        - Logical Rigor: Clear transmission paths showing cause and effect

    When to use:
        - Evaluating financial risk analysis responses
        - Comparing quality of risk assessment
        - Assessing depth of risk quantification
    ...
    Args:
        ...
        template: Custom evaluation template (default: DEFAULT_RISK_ANALYSIS_TEMPLATE)
    ...
    Example:
        ...
        >>> grader = RiskAnalysisGrader(model=model)
        >>>
    ...
    async def _aevaluate(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.exception(f"Error evaluating risk analysis: {e}")
            return GraderError(
                name=self.name,
...
__all__ = ["RiskAnalysisGrader", "DEFAULT_RISK_ANALYSIS_TEMPLATE"]
